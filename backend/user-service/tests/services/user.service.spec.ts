...
describe("UserService - Unit Tests", () => {
  ...
  describe("register", () => {
    ...
    it("should successfully register a new user", async () => {
      ...
      expect(bcryptHashStub.calledWith("password123", BCRYPT_SALT_ROUNDS)).to.be
        .true;
      expect(
        dbPoolQueryStub.calledWith(
    ...
    it("should throw ApiError for duplicate email", async () => {
      bcryptHashStub.resolves("hashed_password");
      const pgError = new pg.DatabaseError(
      ...
      await expect(userService.register(validUser)).to.be.rejectedWith(
        ApiError,
  ...
  describe("login", () => {
    ...
    it("should successfully login and return payload", async () => {
      ...
      expect(bcryptCompareStub.calledWith("password123", "hashed_password")).to
        .be.true;
    });
    ...
    it("should throw ApiError for invalid email", async () => {
      ...
      await expect(userService.login(validLogin)).to.be.rejectedWith(
        ApiError,
        "Email not found",
      );
    });
  ...
  describe("getUserProfile", () => {
    ...
    it("should return user profile", async () => {
      ...
      expect(
        dbPoolQueryStub.calledWith("SELECT * FROM USERS WHERE user_id=$1", [1]),
      ).to.be.true;
    });
    ...
    it("should throw ApiError for non-existent user", async () => {
      ...
      await expect(userService.getUserProfile(1)).to.be.rejectedWith(
        ApiError,
        "User not found",
      );
    });
  ...
  describe("updateProfile", () => {
    const validUpdate = {
      username: "new_name",
      bio: "New bio",
      avatarUrl: "https://newurl.com",
    };

    it("should successfully update profile", async () => {
      dbPoolQueryStub.resolves({ rows: [] });

      await expect(userService.updateProfile(1, validUpdate)).to.eventually.be
        .fulfilled;
      expect(
        dbPoolQueryStub.calledWith(
    ...
    it("should throw ApiError for duplicate username", async () => {
      ...
      await expect(
        userService.updateProfile(1, validUpdate),
      ).to.be.rejectedWith(ApiError, "Username already exists");
    });
  });

  describe("getUserRole", () => {
    it("should return user role", async () => {
      dbPoolQueryStub.resolves({
        rows: [{ role: UserRoleEnum.ADMIN }],
        rowCount: 1,
      });
    ...
    it("should throw ApiError if user not found", async () => {
      ...
      await expect(userService.getUserRole(1)).to.be.rejectedWith(
        ApiError,
        "UserId not found",
      );
    });
  ...
  describe("changeRole", () => {
    it("should allow superadmin to change role", async () => {
      ...
      await expect(
        userService.changeRole(2, { newRole: UserRoleEnum.MODERATOR }, 1),
    ...
    it("should throw ApiError if changer lacks permission to modify higher role", async () => {
      ...
      await expect(
        userService.changeRole(2, { newRole: UserRoleEnum.NORMAL }, 1),
      ).to.be.rejectedWith(
        ApiError,
    ...
    it("should throw ApiError if changer assigns role higher than their own", async () => {
      ...
      await expect(
        userService.changeRole(2, { newRole: UserRoleEnum.SUPERADMIN }, 1),
      ).to.be.rejectedWith(
        ApiError,
