...
export class UserService {
  ...
  async register(newUserDetails: zod.infer<typeof userRegistrationDto>) {
    ...
    const passwordHash = await bcrypt.hash(
      validatedNewUser.password,
      parseInt(BCRYPT_SALT_ROUNDS.toString()),
    );
  ...
  async updateProfile(
    ...
    updatedUserDetails: zod.infer<typeof userUpdateDto>,
  ) {
    ...
    try {
      await dbPool.query(
        ...
        [
          ...
          validatedUpdates.bio,
          validatedUpdates.avatarUrl,
          userId,
        ],
      );
    } catch (err: any) {
  ...
  async changeRole(
    targetUserId: zod.infer<typeof userIdDto>,
    changeRoleDetails: zod.infer<typeof changeRoleDto>,
    changerId: zod.infer<typeof userIdDto>,
  ) {
    ...
    const [changerRole, targetUserCurrentRole] = await Promise.all([
      ...
      await this.getUserRole(validatedTargetUserId),
    ]);

    if (
      changerRole !== UserRoleEnum.SUPERADMIN &&
      this.roleRanks[targetUserCurrentRole] >= this.roleRanks[changerRole]
    ) {
      throw new ApiError(
        403,
    ...
    if (
      changerRole !== UserRoleEnum.SUPERADMIN &&
      this.roleRanks[newRole] >= this.roleRanks[changerRole]
    ) {
